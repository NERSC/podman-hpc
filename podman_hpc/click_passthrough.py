...
class DefaultCommandGroup(click.Group):
    ...
    def set_default_command(self,cmd, name=None):
        if not self.default_command_fn:
            ...
            warnings.warn(f"{self.name}.default_command_fn is already set: {self.default_command_fn}",
                         UserWarning,
                         stacklevel=2)
    ...
    def list_commands(self, ctx: click.Context) -> t.List[str]:
        if self.default_command_fn and self.hide_default:
            return [
                ...
                if not (cmd == self.default_command_fn.name and self.hide_default)
            ]
        else:
...
def customize_help(cls):
    ...
    def get_custom_format(self,ctx):
        if self.custom_format is not None:
            ...
            return getattr(ctx.parent.command,"get_custom_format",lambda x: None)(ctx.parent)
        else:
            return None
    ...
    def format_short_help(self,ctx,formatter):
        ...
        if getattr(self,'short_help',None): formatter.write_text(self.short_help) 

    def format_description(self,ctx,formatter):
        if getattr(self,'description',None) is None:
            self.description = inspect.cleandoc(re.sub("^.+","",self.help)) if self.help else None
