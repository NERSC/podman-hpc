...
class SiteConfig:
    ...
    def __init__(self, squash_dir=None, log_level=None):
        ...
        self.squash_dir = squash_dir or os.environ.get("SQUASH_DIR", f'{os.environ["SCRATCH"]}/storage')
    ...
    def trywhich(cmd,*args,**kwargs):
        ...
        if res is None:
            raise OSError(f"No {cmd} binary found in {kwargs.get('path','PATH')}")
        return res
    ...
    def _write_conf(self, filename, data, overwrite=False):
        ...
        """
        os.makedirs(f"/tmp/containers-user-{self.uid}/containers", exist_ok=True)
