...
def main():
    p = argparse.ArgumentParser(prog='configure_hooks',
            description="Write a hooks.json and install to hooksd for podman-hpc.")
    p.add_argument('-f','--force', action='store_true',
            help='Force overwrite of existing hook file.')
    p.add_argument('--hooksd', type=pathlib.Path, 
            default=os.path.join(sys.prefix,'share','containers','oci','hooks.d'),
